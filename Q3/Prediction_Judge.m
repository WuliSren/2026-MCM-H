%% 与星共舞参赛者特征对排名影响分析 - 随机森林模型（预测第一周评委打分排名）
clear; clc; close all;
...
opts.SelectedVariableNames = {
    ...
    'season',
    'week1_rank',  % 目标变量：第一周评委打分排名
    'fan_vote_pct_combined',  % 特征：粉丝投票百分比
    'placement'  % 特征：最终排名
};
...
% 目标变量：第一周评委打分排名
target = data.week1_rank;
fprintf('第一周评委打分排名统计：最小值=%.2f, 最大值=%.2f, 平均值=%.2f\n', ...
        min(target), max(target), mean(target(target>0)));
...
% 保存模型
save('dwts_week1_rank_random_forest_model.mat', 'rf_model', 'feature_names', ...
...
final_results = table(data.x__elebrity_name, data.season, target, all_pred, ...
                     'VariableNames', {'Celebrity', 'Season', 'Actual_Week1_Rank', 'Predicted_Week1_Rank'});

writetable(final_results, 'dwts_week1_rank_predictions.csv');
fprintf('完整预测结果已保存到: dwts_week1_rank_predictions.csv\n');

% 保存特征重要性
writetable(importance_table, 'feature_importance_week1_rank.csv');
fprintf('特征重要性已保存到: feature_importance_week1_rank.csv\n');

% 保存模型性能
performance_table = table(mae, rmse, r_squared, ...
    'VariableNames', {'MAE', 'RMSE', 'R2'});
writetable(performance_table, 'model_performance_week1_rank.csv');
fprintf('模型性能已保存到: model_performance_week1_rank.csv\n');

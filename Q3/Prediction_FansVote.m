%% 与星共舞参赛者特征对排名影响分析 - 随机森林模型（预测粉丝投票）
clear; clc; close all;
...
% 目标变量：粉丝投票百分比
target = data.fan_vote_pct_combined;
fprintf('粉丝投票百分比统计：最小值=%.2f, 最大值=%.2f, 平均值=%.2f\n', ...
        min(target), max(target), mean(target));

% 定义特征列名（移除粉丝投票，只保留其他6个特征）
feature_columns = {
    'ballroom_partner',
    'celebrity_industry', 
    'celebrity_homestate',
...
% 保存模型
save('dwts_fan_vote_random_forest_model.mat', 'rf_model', 'feature_names', ...
...
final_results = table(data.x__elebrity_name, data.season, target, all_pred, ...
                     'VariableNames', {'Celebrity', 'Season', 'Actual_Fan_Vote', 'Predicted_Fan_Vote'});

writetable(final_results, 'dwts_fan_vote_predictions.csv');
fprintf('完整预测结果已保存到: dwts_fan_vote_predictions.csv\n');

% 保存特征重要性
writetable(importance_table, 'feature_importance_fan_vote.csv');
fprintf('特征重要性已保存到: feature_importance_fan_vote.csv\n');

% 保存模型性能
performance_table = table(mae, rmse, r_squared, ...
    'VariableNames', {'MAE', 'RMSE', 'R2'});
writetable(performance_table, 'model_performance_fan_vote.csv');
fprintf('模型性能已保存到: model_performance_fan_vote.csv\n');
